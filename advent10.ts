...
export const solve = () => {
  ...
  const schematicCoordinates = schematic.map((row, i) => {
    const rowHasStartingPoint = row.includes("S");
    if (rowHasStartingPoint) {
      startingCoordinates = [i, row.indexOf("S")];
    }
  ...
  Object.keys(shapeToDirections).forEach((shape) => {
    if (shape === "." || accum !== 0) {
      return;
    }

    loop = Array.from({ length: schematicCoordinates.length }, () =>
      Array.from({ length: schematicCoordinates[0].length }, () => "X")
    );

    loop[startingCoordinates[0]][startingCoordinates[1]] = shape;

    let canContinue = true;
    let standingCoords = startingCoordinates;
    let standingShape = shape;
    let enterDirection = shapeToDirections[shape][0];
    let nextDirection = shapeToDirections[shape][1];
    let currentSteps = 0;
    ...
    while (canContinue) {
      ...
      try {
        console.log(`standing in a ${standingShape}, entered from ${enterDirection}, going out to ${nextDirection}`);
        if (nextDirection === "N") {
          standingCoords = [standingCoords[0] - 1, standingCoords[1]];
        } else if (nextDirection === "S") {
          standingCoords = [standingCoords[0] + 1, standingCoords[1]];
        } else if (nextDirection === "E") {
          standingCoords = [standingCoords[0], standingCoords[1] + 1];
        } else if (nextDirection === "W") {
          standingCoords = [standingCoords[0], standingCoords[1] - 1];
        }

        standingShape = schematicCoordinates[standingCoords[0]][standingCoords[1]];

        loop[standingCoords[0]][standingCoords[1]] = standingShape;

        if (standingCoords[0] === startingCoordinates[0] && standingCoords[1] === startingCoordinates[1]) {
          standingShape = shape;
          loop[standingCoords[0]][standingCoords[1]] = shape;
        }

        if (!shapeToDirections[standingShape].includes(oppositeMap[nextDirection])) {
          console.log(`shape ${standingShape}, is not compatible with ${nextDirection}`);
          canContinue = false;
        }

        enterDirection = oppositeMap[nextDirection];
        nextDirection = shapeToDirections[standingShape].find((direction) => direction !== enterDirection);

        if (
          standingCoords[0] === startingCoordinates[0] &&
          standingCoords[1] === startingCoordinates[1] &&
          canContinue
        ) {
          console.log(`Missing shape is ${standingShape}`);
          canContinue = false;
          accum = currentSteps;
        }
      } catch {
  ...
  let insideCells = 0;
  loop.forEach((row, rowIdx) => {
    let numOfVerticalPipes = 0;
    let previousTurn = "";
    ...
    row.forEach((cell, cellIndex) => {
      ...
      // An F followed by a J and L followed by a 7 are equivalent to a vertical pipe
      else if (cell === "7" || cell === "L" || cell === "J" || cell === "F") {
        if (previousTurn === "F" && cell === "J") {
          numOfVerticalPipes++;
        } else if (previousTurn === "L" && cell === "7") {
          numOfVerticalPipes++;
        }

        previousTurn = cell;
      } else if (cell === "X") {
        // If there's an odd number of vertical pipe before cell, it's inside the circuit
        if (numOfVerticalPipes % 2 !== 0) {
          console.log("numOfVerticalPipes", numOfVerticalPipes);
          console.log(`cell is inside in coords ${rowIdx} ${cellIndex}`);
          insideCells++;
        }
      }
    });
  ...
  console.log(`inside cells are ${insideCells}`);
  return accum / 2;
};
