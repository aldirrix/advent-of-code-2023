...
// Fix for weird combinations, ascending order
const chainNumberMap = {
  nineight: "98",
  eightwo: "82",
  sevenine: "79",
  fiveight: "58",
...
// Normal replacement after chain fix
const digitStringMap = {
  one: "1",
  two: "2",
  three: "3",
  four: "4",
...
const replaceAll = (value: string): number => {
  let accumulator = value;
  Object.entries({ ...chainNumberMap, ...digitStringMap }).forEach(([key, val]) => {
    accumulator = accumulator.replaceAll(key, val);
  });

  const firstDigit = accumulator.replaceAll(/\D/g, "")[0];
  const lastDigits = accumulator.replaceAll(/\D/g, "");
  const lastDigit = lastDigits[lastDigits.length - 1];

  if (!firstDigit || !lastDigit) {
    console.log("ONE OF THE DIGITS IS undefined!!!");
    return 0;
  }

  console.log(`IN: ${value} OUT: ${firstDigit} NUM: ${parseInt(firstDigit + lastDigit, 10)}`);

  return parseInt(firstDigit + lastDigit, 10);
};

const digitsSeparator = (value: string): number => {
  const replaced = replaceAll(value);
