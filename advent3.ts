...
const isPotentialGear = (char: string) => {
  return char === "*";
};

const hasAdjacentSymbols = (row: string[], numberIndices: number[], prevRow?: string[], nextRow?: string[]) => {
  const firstIndex = numberIndices[0];
  const lastIndex = numberIndices[numberIndices.length - 1];

  // Get one more and one less if possible to check for adjacent symbols in all rows
  const extendedNumberIndices = [
    ...((firstIndex !== 0 && [firstIndex - 1]) || []),
    ...numberIndices,
    ...((lastIndex !== row.length && [lastIndex + 1]) || []),
  ];

  // check if any of the adjacent characters is a symbol
  return extendedNumberIndices.some((index) => {
    let hasSymbolInPrevRow = false;
    let hasSymbolInNextRow = false;
    const hasSymbolInRow = isSymbol(row[index]);

    if (prevRow) {
      hasSymbolInPrevRow = isSymbol(prevRow[index]);
    }
    if (nextRow) {
      hasSymbolInNextRow = isSymbol(nextRow[index]);
    }

    return hasSymbolInPrevRow || hasSymbolInNextRow || hasSymbolInRow;
  });
};
...
export const solve = () => {
  let accum = 0;
  let accum2 = 0;
  const actualPartIndices: number[][][] = [];
  const potentialGearIndices: number[][] = [];

  schematic.forEach((row, i) => {
    actualPartIndices[i] = [];
    let previousRow: string[];
    const currentRow: string[] = row.split("");
    let nextRow: string[];

    if (i !== 0) {
      previousRow = schematic[i - 1].split("");
    }
    if (i !== schematic.length - 1) {
      nextRow = schematic[i + 1].split("");
    }

    const numbersInRowIndices: number[][] = [];
    const potentialGearsInRowIndices: number[] = [];
    // Flag to group numbers when it starts
    let isStarting = true;
    let currentIndex = 0;
    // for each row get all number indices
    currentRow.forEach((char, index) => {
      if (isNumber(char)) {
        if (isStarting) {
          isStarting = false;
          numbersInRowIndices[currentIndex] = [];
        }
        numbersInRowIndices[currentIndex].push(index);
      } else if (isStarting === false) {
        currentIndex += 1;
        isStarting = true;
      }

      if (isPotentialGear(char)) {
        potentialGearsInRowIndices.push(index);
      }
    });

    potentialGearIndices[i] = potentialGearsInRowIndices;

    numbersInRowIndices.forEach((indices) => {
      if (hasAdjacentSymbols(currentRow, indices, previousRow, nextRow)) {
        const completeNumber = parseInt(indices.map((index) => currentRow[index]).join(""), 10);
        accum += completeNumber;

        // Save row actual part indices to reference gears
        actualPartIndices[i].push(indices);
      }
    });
  });

  potentialGearIndices.forEach((potentialGearsInRow, rowIndex) => {
    potentialGearsInRow.forEach((potentialGearIndex) => {
      let adjacentParts: number[] = [];
      let prevRowParts: number[][] = [];
      const currentRowParts = actualPartIndices[rowIndex];
      let nextRowParts: number[][] = [];

      if (rowIndex !== 0) {
        prevRowParts = actualPartIndices[rowIndex - 1];
      }

      if (rowIndex !== schematic.length - 1) {
        nextRowParts = actualPartIndices[rowIndex + 1];
      }

      prevRowParts.forEach((part) => {
        const isPartAdjacent =
          part.includes(potentialGearIndex) ||
          part.includes(potentialGearIndex - 1) ||
          part.includes(potentialGearIndex + 1);

        if (isPartAdjacent) {
          const partVal = parseInt(
            part
              .map((partIndex) => {
                return schematic[rowIndex - 1][partIndex];
              })
              .join("")
          );
          adjacentParts.push(partVal);
        }
      });

      currentRowParts.forEach((part) => {
        const isPartAdjacent =
          part.includes(potentialGearIndex) ||
          part.includes(potentialGearIndex - 1) ||
          part.includes(potentialGearIndex + 1);
        ...
        if (isPartAdjacent) {
          const partVal = parseInt(
            part
              .map((partIndex) => {
                return schematic[rowIndex][partIndex];
              })
              .join("")
          );
          adjacentParts.push(partVal);
      ...
      if (adjacentParts.length === 2) {
        accum2 += adjacentParts[0] * adjacentParts[1];
      }
    });
  });

  console.log(console.log(accum));
  return accum2;
};
